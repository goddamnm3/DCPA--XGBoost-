"""
口腔健康风险预测系统 - 完整实现
该算法使用随机森林作为预测模型，集成了特征工程、模型优化和解释性功能
...
from sklearn.preprocessing import LabelEncoder, KBinsDiscretizer  # 数据预处理
...
class DentalHealthPredictor:
    ...
    def optimize_parameters(self, X_train, y_train, X_val, y_val):
        ...
        param_grid = {
            ...
            'max_depth': [4, 6, 8],      # 减少深度选项
            'min_samples_split': [2, 4],  # 减少分裂样本数选项
            'min_samples_leaf': [1, 2],   # 减少叶子节点样本数选项
            'max_features': ['sqrt', 'log2'],  # 减少特征选择方法
            'class_weight': ['balanced', 'balanced_subsample'],
            'ccp_alpha': [0.0, 0.01]     # 减少剪枝参数选项
        }
        ...
        # 计算总拟合度
        total_fit = train_score + val_score
        print(f"总拟合度: {total_fit:.4f}")
